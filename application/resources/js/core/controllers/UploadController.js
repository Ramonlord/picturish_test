...
angular.module('app').controller('UploadController', ['$rootScope', '$scope', '$translate', '$upload', '$state', 'folders', 'photos', 'activity', 'utils', 'users', 'localStorage', function($rootScope, $scope, $translate, $upload, $state, folders, photos, activity, utils, users, localStorage) {
    ...
    $scope.validExtensions = utils.getSetting('validExtensions').replace(/ /g,'').split(',');
    ...
    $scope.upload = function (files) {
        ...
        if ( ! files || ! files.length || ! files[0].type) return;
    ...
    function fileNotValid(file) {
        ...
        var extensionNotValid = $scope.validExtensions.indexOf(extension) === -1;
    ...
    function sendRequest(file, historyItem) {
        $upload.upload({
            ...
            if (data.uploaded.length) {
                folders.selected.photos = folders.selected.photos.concat(data.uploaded);
            ...
            if ($scope.uploadsInProgress === 0 && users.current) {
                $rootScope.$emit('activity.happened', 'uploaded', 'photo', $scope.uploadHistory.slice());
            }
        });
    }
}])
